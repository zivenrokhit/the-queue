...
function submitPlaylist(){
  ...
  <div id="create-playlist">
    ...
    <input type="text" onChange={e => setPlaylistName(e.target.value)}/>
      <div id="search-songs">
        ...
        <input type="text" placeholder="Search.." onChange={e => SearchSongs(e.target.value)}/>
        
        <div className="dropdown">
          {searchResults.map((result, idx) => (
            <p key={idx} onMouseDown={() => addSong(result)}>         {/* This needs to be onMouseDown otherwise the dropdown menu hides before a click is registered */}
  ...
  <div id="song-list">
    <h3>Songs in Playlist:
      {songsList.map((song, idx) => (
        <p key={idx}>{song.title} | {song.artist}</p>
      ))}
    </h3>
  </div>  
